...
enc_params = sum(p.view(-1).size(0) for p in enc.parameters())
print("enc params: ", enc_params)

dec_params = sum(p.view(-1).size(0) for p in dec.parameters())
...
with torch.no_grad():
    for idx in range(len((input_lengths_test))):
        ...
        out, gate, attn = evaluate_with_attn(input_batches_test[idx], input_lengths_test[idx], output_batches_test[idx],
