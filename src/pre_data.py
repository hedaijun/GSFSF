...
from src.expressions_transfer import out_expression_list, compute_prefix_expression, from_infix_to_prefix
...
def transfer_num(data, isTrans=False):  # transfer num into "NUM"
    print("Transfer numbers...")
...
def indexes_from_sentence(lang, sentence, tree=False, isTrans=False):
    res = []
...
def prepare_data_bert(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, file, tree=False,
    ...
    for pair in pairs_trained:
        ...
        input_cell = indexes_from_sentence(input_lang, pair[0], isTrans=isTrans)
...
def prepare_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    for pair in pairs_tested:
        ...
        num_stack.reverse()
        input_cell = indexes_from_sentence(input_lang, pair[0], isTrans=False)
        output_cell = indexes_from_sentence(output_lang, pair[1], tree=tree)
        # train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
        #                     pair[2], pair[3], num_stack, pair[4]))
        test_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
